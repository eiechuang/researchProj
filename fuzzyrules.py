...
#for fuzzy rule comparison to straight edge 4 IBMparser2.py
...
OUTPUT_FILE = DATA_DIR + r"/fuzzy_final_ibm_bank.csv"
 
  
 
def fuzzy_ramp_up(x, low, high):
    if pd.isna(x):
        return 0.0
...
def fuzzy_ramp_down(x, low, high):
    if pd.isna(x):
        return 0.0
...
def fuzzy_triangle(x, left, peak, right):
    if pd.isna(x):
        return 0.0
...
def fuzzy_roundness(x, base=1000):
    if pd.isna(x):
        return 0.0
 
    remainder = x % base
    distance_to_round = min(remainder, base - remainder)
 
    return max(0.0, 1.0 - (distance_to_round / (base / 2)))
...
def add_transaction_features(df):
    ...
    df["fuzzy_near_10000"] = df["amount_paid"].apply(
        lambda x: fuzzy_triangle(x, left=8000, peak=9900, right=11000)
    )
 
    df["fuzzy_large_amount"] = df["amount_paid"].apply(
        lambda x: fuzzy_ramp_up(x, low=7500, high=25000)
    )
 
    df["fuzzy_very_large_amount"] = df["amount_paid"].apply(
        lambda x: fuzzy_ramp_up(x, low=50000, high=250000)
    )
 
    df["fuzzy_round_1000"] = df["amount_paid"].apply(
        lambda x: fuzzy_roundness(x, base=1000)
    )
 
    df["fuzzy_round_10000"] = df["amount_paid"].apply(
        lambda x: fuzzy_roundness(x, base=10000)
    )
 
    df["fuzzy_amount_mismatch"] = df["amount_difference"].apply(
        lambda x: fuzzy_ramp_up(x, low=0, high=1000)
    )
...
def route_transaction(row):
    ...
    if row["fuzzy_near_10000"] >= 0.7 and row["same_bank"] == 0:
    ...
    if row["fuzzy_large_amount"] == 1: #new broken
...
def add_behavior_features(df):
    ...
    df["sender_tx_count"] = (
        df.groupby("from_account")["from_account"].transform("count")
    )
 
    df["receiver_tx_count"] = (
        df.groupby("to_account")["to_account"].transform("count")
    )
 
    df["sender_total_amount"] = (
        df.groupby("from_account")["amount_paid"].transform("sum")
    )
 
    df["receiver_total_amount"] = (
        df.groupby("to_account")["amount_paid"].transform("sum")
    )
 
    df["sender_avg_amount"] = (
        df.groupby("from_account")["amount_paid"].transform("mean")
    )
 
    df["receiver_avg_amount"] = (
        df.groupby("to_account")["amount_paid"].transform("mean")
    )
    ...
    df["sender_unique_receivers"] = (
        df.groupby("from_account")["to_account"].transform("nunique")
    )
 
    df["receiver_unique_senders"] = (
        df.groupby("to_account")["from_account"].transform("nunique")
    )
 
    df["sender_daily_tx_count"] = (
        df.groupby(["from_account", "date"])["from_account"].transform("count")
    )
 
    df["receiver_daily_tx_count"] = (
        df.groupby(["to_account", "date"])["to_account"].transform("count")
    )
 
    df["sender_daily_total"] = (
        df.groupby(["from_account", "date"])["amount_paid"].transform("sum")
    )
 
    df["receiver_daily_total"] = (
        df.groupby(["to_account", "date"])["amount_paid"].transform("sum")
    )
 
    df["fuzzy_fan_out"] = df["sender_unique_receivers"].apply(
        lambda x: fuzzy_ramp_up(x, low=2, high=10)
    )
 
    df["fuzzy_fan_in"] = df["receiver_unique_senders"].apply(
        lambda x: fuzzy_ramp_up(x, low=2, high=10)
    )
 
    df["fuzzy_sender_burst"] = df["sender_daily_tx_count"].apply(
        lambda x: fuzzy_ramp_up(x, low=2, high=8)
    )
 
    df["fuzzy_receiver_burst"] = df["receiver_daily_tx_count"].apply(
        lambda x: fuzzy_ramp_up(x, low=2, high=8)
    )
 
    df["fuzzy_amount_vs_sender_avg"] = df["amount_vs_sender_avg"].apply(
        lambda x: fuzzy_ramp_up(x, low=1.5, high=5.0)
    )
 
    df["fuzzy_amount_vs_receiver_avg"] = df["amount_vs_receiver_avg"].apply(
        lambda x: fuzzy_ramp_up(x, low=1.5, high=5.0)
    )
...
def add_engineered_risk_score(df):
    ...
    df["structuring_score"] = (
        df["fuzzy_near_10000"] * 2
        + ((df["same_bank"] == 0).astype(int))
    )
 
    df["fan_out_score"] = (
        df["fuzzy_fan_out"] * 2
        + df["fuzzy_sender_burst"]
    )
 
    df["fan_in_score"] = (
        df["fuzzy_fan_in"] * 2
        + df["fuzzy_receiver_burst"]
    )
 
    df["layering_score"] = (
        (df["same_bank"] == 0).astype(int)
        + df["currency_changed"].astype(int)
        + df["fuzzy_amount_mismatch"]
    )
